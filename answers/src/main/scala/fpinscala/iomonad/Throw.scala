...
/** A version of `TailRec` implemented using exceptions. In the implementation of `flatMap`, rather than calling the
  * function, we throw an exception indicating what function we want to call. A central loop repeatedly tries and
  * catches these exceptions to force the computation.
  */
trait Throw[+A] {
  import Throw._
