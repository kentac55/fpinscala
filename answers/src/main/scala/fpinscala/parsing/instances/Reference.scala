...
object ReferenceTypes {
  ...
  /** `ParseState` wraps a `Location` and provides some extra convenience functions. The sliceable parsers defined in
    * `Sliceable.scala` add an `isSliced` `Boolean` flag to `ParseState`.
    */
