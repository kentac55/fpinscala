...
trait Parsers[Parser[+_]] { self => // so inner classes may call methods of trait
  ...
  /** Sequences two parsers, ignoring the result of the first. We wrap the ignored half in slice, since we don't care
    * about its result.
    */
  ...
  /** C/Java style floating point literals, e.g .1, -1.0, 1e9, 1E-23, etc. Result is left as a string to keep full
    * precision
    */
...
case class ParseError(stack: List[(Location, String)] = List()) {
  ...
  /** Display collapsed error stack - any adjacent stack elements with the same location are combined on one line. For
    * the bottommost error, we display the full line, with a caret pointing to the column of the error. Example:
    *
