...
/** Processes messages of type `A`, one at a time. Messages are submitted to the actor with the method `!`. Processing
...
object Strategy {
  ...
  /** We can create a `Strategy` from any `ExecutorService`. It's a little more convenient than submitting `Callable`
    * objects directly.
    */
